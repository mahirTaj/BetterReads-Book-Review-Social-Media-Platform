...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["submit"])) {
    if (!empty($_POST["search"])) {
        ...
        if ($search_type == 'book') {
            ...
            $stmt = $conn->prepare("SELECT * FROM user u INNER JOIN author a ON u.user_id = a.author_id WHERE LOWER(u.fname) LIKE ? OR LOWER(u.mname) LIKE ? OR LOWER(u.lname) LIKE ?");
            $stmt->bind_param("sss", $search, $search, $search);
        } elseif ($search_type == 'people') {
            $stmt = $conn->prepare("SELECT * FROM user u INNER JOIN reader r ON u.user_id = r.reader_id WHERE LOWER(u.fname) LIKE ? OR LOWER(u.mname) LIKE ? OR LOWER(u.lname) LIKE ?");
            $stmt->bind_param("sss", $search, $search, $search);
        }
            ...
            if ($result->num_rows > 0) {
                ...
                while ($row = $result->fetch_assoc()) {
                    ...
                    if ($search_type == 'book') {
                        ...
                        echo "<img src='{$row["profile_picture"]}' alt='Author Picture'>";
                        echo "<h3><a href='visit_author.php?author_id={$row["user_id"]}'>{$full_name}</a></h3>";
                    } elseif ($search_type == 'people') {
                        $full_name = trim("{$row["fname"]} {$row["mname"]} {$row["lname"]}");
                        echo "<img src='{$row["profile_picture"]}' alt='Reader Picture'>";
